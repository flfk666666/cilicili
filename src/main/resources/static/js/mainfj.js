...
(function ($) {
    ...
    var DanmuPlayer = function (element, options) {
        ...
        this.$ctrlMain.append('<div class="show-danmu  ctrl-btn-right ctrl-btn-right-active" style="display:inline-block; width:100px;"><span class="" aria-hidden="true">关闭弹幕</span></div>');
        this.$ctrlMain.append('<div><select id="colortest" style="height: 30px;width: 50px">\n' +
        ...
        this.sendDanmu = function (e) {
            ...
            if (uid==0||uid==null||uid==""){
                var ynlogin=confirm("您还未登录，请先登录");
                if (ynlogin) {
                    location.href="fj/fidlogin?fid="+fid+"&&"+"fnumber="+fnumber;
                }else {

                }
            }else {
                $.post("fj/adddm",{time:time,text:text,color:color,fid:fid,uid:uid,fnumber:fnumber},function(data){
